...
export const PlanCard = ({ plan, isCompact = false, orientation = "vertical", noBorderRadius = false, enableFlip = false, enableExpand = false }: { plan: PlanProps; isCompact?: boolean; orientation?: "vertical" | "horizontal"; noBorderRadius?: boolean; enableFlip?: boolean; enableExpand?: boolean }) => {
  ...
  // Restore hasAddOn state from cart on mount for 30 Days Content Plans
  useEffect(() => {
    if (plan.id === "30-days-content" || plan.id === "30-days-content-part-2") {
      const existingItem = items.find((item) => item.id.startsWith(plan.id));
      if (existingItem && existingItem.id.includes('-addon')) {
        setHasAddOn(true);
      } else if (existingItem && !existingItem.id.includes('-addon')) {
        // If plan is in cart but without addon, ensure hasAddOn is false
        setHasAddOn(false);
      }
    }
  }, [items, plan.id]);
  ...
  const isSelected = (plan.id === "30-days-content" || plan.id === "30-days-content-part-2")
    ? items.some((item) => item.id.startsWith(plan.id))
    : items.some((item) => item.id === cartItemId);

  const handleFlipToFeatures = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleToggleSelection = (e: React.MouseEvent) => {
    ...
    if (isSelected) {
      ...
      if (enableExpand) setIsExpanded(false);
      toast({
        title: "Plan Removed",
      ...
      if (enableExpand) setIsExpanded(true);
    }
  };
  ...
  // Flip card for Digital Content Creation
  if (enableFlip) {
    return (
      <div 
        ref={cardRef}
      ...
      >
        ...
        >
          ...
          >
            <SpotlightCard 
              className={`h-full shadow-sm p-0 ${isSelected ? "ring-2 ring-primary" : ""}`}
              clickEffect={false}
            >
              {plan.badge && (
              ...
              <div className={`absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r ${plan.gradient}`} />
              ...
              <div className="p-5 flex flex-col h-full">
                ...
                <div className="py-3 border-y border-border/50">
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-bold text-foreground">₹{(dynamicPrice + (hasAddOn ? 2500 : 0)).toLocaleString()}</span>
                    <span className="text-muted-foreground text-sm">/month</span>
                  </div>
                </div>
                <div className="flex-1 flex items-center justify-center py-4">
                  <p className="text-sm text-muted-foreground text-center">Click below to see what's included</p>
                </div>
                <div className="mt-auto">
                  <Button 
                    className={`w-full ${isSelected ? "bg-green-600 hover:bg-green-700 text-white" : ""}`}
                    size="sm"
                    onClick={isSelected ? handleToggleSelection : handleFlipToFeatures}
                    disabled={plan.disabled}
                  >
                    {isSelected ? "Selected ✓" : plan.disabled ? "Coming Soon" : "View Details"}
                  </Button>
                </div>
              </div>
            </SpotlightCard>
          ...
          >
            <div className={`relative h-full bg-card border border-border rounded-3xl shadow-sm overflow-hidden`}>
              {/* Gradient glow effect */}
              <div className={`absolute inset-0 opacity-20 rounded-3xl pointer-events-none`} style={{ background: `linear-gradient(135deg, ${plan.gradient.includes('pink') ? 'rgba(236, 72, 153, 0.15)' : plan.gradient.includes('orange') ? 'rgba(249, 115, 22, 0.15)' : plan.gradient.includes('purple') ? 'rgba(168, 85, 247, 0.15)' : plan.gradient.includes('blue') ? 'rgba(59, 130, 246, 0.15)' : plan.gradient.includes('red') ? 'rgba(239, 68, 68, 0.15)' : 'rgba(139, 92, 246, 0.15)'} 0%, transparent 50%)` }} />
              <div className={`absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r ${plan.gradient}`} />
              ...
              <div className="p-5 flex flex-col h-full">
                ...
                <button 
                  ...
                  className="absolute top-3 right-3 p-2 rounded-full bg-secondary hover:bg-secondary/80 transition-colors z-10"
                >
                  <RotateCcw className="w-4 h-4 text-foreground" />
                </button>

                <h4 className="text-base font-bold text-foreground mb-4">What You Get:</h4>
  ...
  return (
    <SpotlightCard key={plan.id} noBorderRadius={noBorderRadius} className={`bg-card border-border shadow-sm hover:shadow-xl transition-all duration-300 relative group flex p-0 ${isSelected ? "ring-2 ring-primary" : ""} ${orientation === "horizontal" ? "md:flex-row" : "flex-col"} ${enableExpand && isExpanded ? "scale-105 z-10" : "lg:hover:scale-[1.02]"}`}>
      {plan.badge && (
        <div className="absolute top-0 right-0 bg-primary text-primary-foreground text-[10px] font-bold px-3 py-1 rounded-bl-xl z-20">
          {plan.badge}
        </div>
      ...
      <div className={`${isCompact ? "p-4" : "px-4 py-6"} flex ${orientation === "horizontal" ? "flex-col md:flex-row gap-6 w-full items-center" : "flex-col h-full"}`}>
        ...
        {/* Features Section */}
        <div className={`${orientation === "horizontal" ? "flex-1 w-full" : "space-y-4 mb-8 flex-grow"} ${enableExpand && !isExpanded ? "hidden" : ""}`}>
            {enableExpand && isExpanded && <h4 className="font-bold mb-2 px-1">What You Get:</h4>}
        ...
        <div className={`${orientation === "horizontal" ? "w-full md:w-auto md:min-w-[150px] flex flex-col justify-center" : "mt-auto"}`}>
            ...
            disabled={plan.disabled}
            >
            {isSelected ? "Selected" : plan.disabled ? "Coming Soon" : (enableExpand ? "Add to Cart" : "Select")}
            </Button>
        </div>
      </div>
    </SpotlightCard>
